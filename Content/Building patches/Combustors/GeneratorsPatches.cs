...
namespace Rephysicalized
{
    ...
    public static class MethaneGenerator_OxygenConsumerAndFormulaPatch
    {
        ...
        public static void Postfix(GameObject go)
        {
            ...
            if (energyGen != null)
            {
                var currentInputs = energyGen.formula.inputs.ToList();
                currentInputs.RemoveAll(input => input.tag == ModTags.OxidizerGas);
                currentInputs.Add(new EnergyGenerator.InputItem(ModTags.OxidizerGas, 0.06f, 1f));
                energyGen.formula.inputs = currentInputs.ToArray();

                energyGen.formula.outputs = new EnergyGenerator.OutputItem[2]
                {
      ...
      SimHashes.ContaminatedOxygen,     };
        ...
        public static class PetroleumGenerator_AddToDescription
        {
            public static void Prefix()
            {
                ...
                Strings.Add(key, Strings.Get(key) + "\n\n <b>REPHYSICALIZED: Requires Oxidizer gas input pipe</b>");
            }
        }

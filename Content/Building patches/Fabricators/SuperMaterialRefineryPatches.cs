...
namespace Rephysicalized.Content.Building_patches
{
    ...
    nameof(SupermaterialRefineryConfig.ConfigureBuildingTemplate))]
    internal static class SupermaterialRefineryConfigUrainumpowerbank
    {
        public static void Postfix(GameObject go, Tag prefab_tag)
        {
            var crm = ComplexRecipeManager.Get();
            var vanillaInputs = new[] { new ComplexRecipe.RecipeElement(SimHashes.EnrichedUranium.CreateTag(), 10f),
              };
            var vanillaOutputs = new[] { new ComplexRecipe.RecipeElement("SelfChargingElectrobank", 1f, ComplexRecipe.RecipeElement.TemperatureOperation.AverageTemperature) };
            string vanillaId = ComplexRecipeManager.MakeRecipeID(SupermaterialRefineryConfig.ID, vanillaInputs, vanillaOutputs);
            ...
            new ComplexRecipe.RecipeElement(SimHashes.EnrichedUranium.CreateTag(), 10f),
                  new ComplexRecipe.RecipeElement(GameTags.BasicRefinedMetals, 10f)
        };
        }
    }
    ...
    public static class SupermaterialRefineryConfig_AddDUtoLeadRecipe_Patch
    {
        public static void Postfix(GameObject go, Tag prefab_tag)
        {
                ...
                {
                    new ComplexRecipe.RecipeElement(SimHashes.DepletedUranium.CreateTag(), 100f)
                };

...
namespace Rephysicalized.Content.Animal_patches
{
       ...
       private const float RAW_KCAL_PER_KG = 1600000f;
        ...
        static void Postfix(SolidConsumerMonitor.Instance __instance, object data)
        {
            GameObject eater = __instance?.smi?.gameObject;
                ...
                // Vanilla stomach uses calories as its limiting resource. If the overflow is computed too aggressively
                // (e.g. because calSnapshot already includes other diet effects), it can produce absurdly large meat.
                // Clamp meat mass to a reasonable bound: at most the raptor-poop “overfeed” should never exceed the prey mass itself.
                float preyKg = startingMass;
                ...
                float maxMeatKg = Mathf.Max(0f, preyKg);
                float rawOverfedKg = overfedCalories / Mathf.Max(1f, RAW_KCAL_PER_KG);
                if (rawOverfedKg > maxMeatKg)
                {
                    overfedCalories = maxMeatKg * RAW_KCAL_PER_KG;
                }
    ...
    // Buffer component to hold pending extra poop mass for Raptors
    ...
    internal static class RaptorPoopUtil
    {
        ...
        internal static float GetOwnerTemperature(GameObject owner)
        {
            return owner.GetComponent<PrimaryElement>()?.Temperature ?? 243.15f;
        }

        internal static bool ForceClearAndSpawnPoop(GameObject owner, RaptorExtraPoopBuffer buffer)
        {
            if (owner == null || buffer == null) return false;
            if (buffer.pendingKg <= 0f) return false;
            if (!buffer.poopElement.IsValid) return false;
            ...
            float temperature = GetOwnerTemperature(owner);

            // Mirror logic from RaptorStomachPoopExtraSpawnPatch: spawn as liquid/gas/solid.
            bool storePoop = false;
            // If we cannot access the stomach instance here, default to immediate spawn (storePoop=false)
            // which matches the non-store branch in your existing implementation.
    ...
    internal static class RaptorExtraPoopOnEatPatch
    {
        ...
        static void Prefix(SolidConsumerMonitor.Instance __instance, object data)
        {
            try
            {
                ...
                // Raptors poop is Brine Ice in this mod.
                buffer.poopElement = SimHashes.BrineIce.CreateTag();
                buffer.pendingKg += extraPoopKg;

                // IMPORTANT: vanilla Stomach.Poop may not fire reliably once diet logic is changed.
                // If we have enough buffered poop to matter, force-spawn it right now.
                // This preserves “poop depends on prey mass”, while ensuring it always happens.
                if (buffer.pendingKg >= 1f)
                {
                    // Force-spawn the buffered brine ice poop immediately.
                    RaptorPoopUtil.ForceClearAndSpawnPoop(eaterGO, buffer);
                }
            }
            catch (Exception e)
            {
    ...
    // When the raptor poops (stomach event), spawn buffered extra poop mass 
    ...
    internal static class RaptorStomachPoopExtraSpawnPatch
    {
        ...
        static void Postfix(CreatureCalorieMonitor.Stomach __instance)
        {
            try
            {
                ...
                float temperature = owner.GetComponent<PrimaryElement>()?.Temperature ?? 243.15f;

...
namespace Rephysicalized
{
    ...
    public static class LightBugDietTranspiler
    {
        // Adds sulfur and DLC1-gated swamp tags to the consumed_tags HashSet unless prefab is excluded
        public static void EnsureExtraTags(GameObject prefab, HashSet<Tag> consumed_tags)
        {
            var kpid = prefab.GetComponent<KPrefabID>();
            if (kpid != null)
            {
                var pt = kpid.PrefabTag;
                if (pt == (Tag)"LightBugCrystal" || pt == (Tag)"LightBugBlack")
                    return;
            }
        ...
        [HarmonyPatch(typeof(BaseLightBugConfig), nameof(BaseLightBugConfig.SetupDiet))]
        public static class BaseLightBugConfig_SetupDiet_Transpiler
        {
            static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
            {
                var code = new List<CodeInstruction>(instructions);
                var ensureMethod = AccessTools.Method(typeof(LightBugDietTranspiler), nameof(EnsureExtraTags));

                // Insert at start: Ldarg_0 (prefab), Ldarg_1 (consumed_tags), Call EnsureExtraTags
                code.InsertRange(0, new[] {
                    new CodeInstruction(OpCodes.Ldarg_0),
                    new CodeInstruction(OpCodes.Ldarg_1),
                    new CodeInstruction(OpCodes.Call, ensureMethod)
                });

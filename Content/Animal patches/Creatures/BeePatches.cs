...
[HarmonyPatch(typeof(BeeForageStates), nameof(BeeForageStates.StoreOre))]
internal static class BeeDeliveryMassTransfer_Patch
{
    public static void Postfix(BeeForageStates.Instance smi)
    {
        if (smi == null || smi.master == null) return;

        var beeGO = smi.master.gameObject;
        var peBee = beeGO.GetComponent<PrimaryElement>();
        if (peBee == null || peBee.Mass <= 1f) return;

        float oldMass = peBee.Mass;
        float removed_mass = Mathf.Max(0f, oldMass - 1f);

        var trackerBee = beeGO.GetComponent<CreatureMassTracker>();
        if (trackerBee != null)
        {
            trackerBee.AddExternalMass(-removed_mass);
        }
        else
        {
            peBee.Mass = 1f;
        ...
        // Fallback find hive if targetHive null
        KPrefabID targetHive = smi.targetHive ?? smi.master.GetComponent<Bee>().FindHiveInRoom();
        if (targetHive == null)
        {
          //  Debug.LogWarning($"BeeDeliveryMassTransfer: No targetHive (fallback FindHiveInRoom failed) for bee {beeGO.name}");
            return;
        }

        var hiveGO = targetHive.gameObject;
        var trackerHive = hiveGO.GetComponent<CreatureMassTracker>();
        if (trackerHive != null)
        {
            trackerHive.AddExternalMass(removed_mass);
          //  Debug.Log($"BeeDeliveryMassTransfer: Added {removed_mass:F2}kg to hive tracker {hiveGO.name}");
        }
        else
        {
            ...
            if (peHive != null)
            {
                peHive.Mass += removed_mass;
           //     Debug.Log($"BeeDeliveryMassTransfer: Fallback added {removed_mass:F2}kg to hive PE {hiveGO.name}");
            }
            else
            {
              //  Debug.LogWarning($"BeeDeliveryMassTransfer: No tracker or PE on hive {hiveGO.name}");
            }
        }

 //       Debug.Log($"BeeDeliveryMassTransfer: Bee {beeGO.name} mass {oldMass:F2}kg -> {peBee.Mass:F2}kg, transferred {removed_mass:F2}kg");
    }
}
}
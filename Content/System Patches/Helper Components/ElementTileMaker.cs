...
public class ElementTileMaker : KMonoBehaviour
{
    [SerializeField] public Tag emitTag;
    [SerializeField] public float emitMass;
    [SerializeField] public bool MakeTiles = true;
    ...
    private static readonly EventSystem.IntraObjectHandler<ElementTileMaker> OnStorageChangedDelegate =
        new EventSystem.IntraObjectHandler<ElementTileMaker>((component, data) => component.OnStorageChanged(data));
    ...
    public void OnStorageChanged(object _)
    {
        if (storage == null || emitMass <= 0f || emitTag == Tag.Invalid)
            return;

        // When not making tiles, behave like an element dropper
        if (!MakeTiles)
        {
            if (storage.GetMassAvailable(emitTag) < emitMass)
                return;
            storage.DropSome(emitTag, emitMass, offset: emitOffset, showInWorldNotification: true);
            return;
        }
...
public static class ComplexFabricator_CompleteWorkingOrder_ElementTileMaker_Patch
{
    public static void Postfix(ComplexFabricator __instance)
    {
        var tileMaker = __instance.GetComponent<ElementTileMaker>();
        if (tileMaker != null)
            tileMaker.OnStorageChanged(null);
    }
}
